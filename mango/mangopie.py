...
class Mango:
    ...
    def get_relationTypes(self):
        ...
        all_types = {
            r["name"] + 
            (r["type"]["name"] if r["type"] else "") + 
            (r["relationType"]["name"] if r["relationType"] else "") : r["_id"] 
            for r in response
        }
            ...
            try:
                ...
                if result:
                    if '_id' in result[0]:
                        ...
                        #print(f"Missing '_id' for the entity with parameters {params_dict}")
                        return None
                else:
                    #print(f"No results found for the query with parameters {params_dict}")
                    return None
            except (requests.exceptions.RequestException, KeyError):
                if attempt < 4:  # retry 5 times, hence < 4
